...
public class Rect {
    ...
    public boolean isMe(Vector2 touch) { //определяет, попадает ли вектор в наш прямоугольник
        return touch.x >= getLeft() && touch.x <= getRight() && touch.y >= getBottom() && touch.y <= getTop();
    }

    public boolean isOutside(Rect other) {  //проверяет пересечение с другим прямоугольником
